...
class DefaultVideoClientControllerTest {
    ...
    @Test
    fun `startLocalVideo(config) should start camera capture and then video client`() {
        every { mockVideoClientStateController.canAct(any()) } returns true
    ...
    @Test
    fun `startLocalVideo(source) should not start camera capture and then video client`() {
        every { mockVideoClientStateController.canAct(any()) } returns true
    ...
    @Test
    fun `startLocalVideo(source, config) should not start camera capture and then video client`() {
        every { mockVideoClientStateController.canAct(any()) } returns true
        val localVideoConfig = LocalVideoConfiguration(600)
